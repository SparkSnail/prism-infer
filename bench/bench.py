...
def _build_llm(path: str, eager: bool, max_model_len: int) -> LLM:
    # MoE must run eager (CUDA graph not supported on the MoE path yet).
    llm = LLM(path, enforce_eager=eager, tensor_parallel_size=1, max_model_len=max_model_len)
...
def cmd_throughput(args):
    ...
    llm = _build_llm(path, args.eager, args.max_model_len)
    prompts, params = _make_requests(args.num_seqs, args.input_len, args.output_len)
    r = _run(llm, prompts, params)
    print(f"\n=== throughput ({'eager/MoE' if args.eager else 'cuda-graph'}) ===")
...
def cmd_sweep(args):
    ...
    print(f"\n=== sweep ({'eager/MoE' if args.eager else 'cuda-graph'}) input_len={args.input_len} output_len={args.output_len} ===")
