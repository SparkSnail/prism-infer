...
The first public container release follows package version `v0.3.0`:

| Image | Profile |
| --- | --- |
| `sparksnail/prism-infer:v0.3.0` | Qwen3-0.6B correctness worker |
| `sparksnail/prism-infer:v0.3.0-qwen3-8b` | Qwen3-8B performance worker |

Tags are release aliases. Pin an image digest together with the matching source commit when deploying outside a local experiment or collecting paired benchmark evidence. Do not use `latest`.
...
Create the Git tag `v0.3.0` on the same clean source commit used for the published image.
